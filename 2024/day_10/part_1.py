...
def walk(x, y, level):
    if (x,y) in level_coords[9]: return {(x,y)}
    return set().union(*[walk(*neigh, level+1) for neigh in valid_neighs(x, y, level)])

print('Part 1:', sum([len(walk(x, y, 0)) for x,y in level_coords[0]]))
